# -*- coding: utf-8 -*-
"""
Created on Mon Feb 10 13:29:36 2020

@author: Yen-Chieh, Huang

Define loss functions for MCMC
...
class DBI(nn.Module):
    def __init__(self, num_of_clus, point_dim, gpu_stat):
        ...
        if(gpu_stat):
            self.clus_count = torch.ones(num_of_clus).cuda()*0.001
            ...
            self.clus_count = torch.ones(num_of_clus)*0.001
    ...
    def forward(self, data_points, clustering):
        ...
        for count in range(self.num_of_clus):
            if(self.clus_count[count]!=0):
                self.Ai[count] = torch.div(self.Ai[count].clone(),self.clus_count[count])
#         print(data_points)
...
#         print(self.Ai)
        ...
        for count in range(self.num_of_clus):
            self.Si[count] = torch.pow(torch.div(self.Si[count].clone(),self.clus_count[count]),0.5)
        #print(self.Si)
        ### Compute Mij
            ...
            for j in range(self.num_of_clus):
                ...
                if(i<j):
                    ...
                    if(self.Mij[i][j]==0):
                        ...
                        self.Rij[i][j] = torch.div(self.Si[i]+self.Si[j], self.Mij[i][j])
